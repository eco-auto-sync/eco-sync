...
(
    ...
    email          VARCHAR(255) NOT NULL                 COMMENT '구독자 이메일',
    calendar_token CHAR(36)     NOT NULL                 COMMENT 'ICS URL 식별자 (UUID)',
    created_by     VARCHAR(255) NULL                     COMMENT '생성자',
    created_at     DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일시',
    updated_by     VARCHAR(255) NULL                     COMMENT '수정자',
    updated_at     DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일시',
    deleted_by     VARCHAR(255) NULL                     COMMENT '삭제자',
    deleted_at     DATETIME     NULL                     COMMENT '삭제일시 (NULL = 활성)',
    PRIMARY KEY (id),
...
(
    id              BIGINT      NOT NULL AUTO_INCREMENT  COMMENT '구독 관심사 PK',
    subscription_id BIGINT      NOT NULL                 COMMENT '구독 FK',
    interest_type   VARCHAR(20) NOT NULL                 COMMENT '관심사 유형 (COUNTRY | IMPORTANCE | TICKER)',
    interest_value  VARCHAR(50) NOT NULL                 COMMENT '관심사 값 (KR, US | HIGH, MEDIUM | AAPL)',
    created_by      VARCHAR(255) NULL                    COMMENT '생성자',
    created_at      DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성일시',
    updated_by      VARCHAR(255) NULL                    COMMENT '수정자',
    updated_at      DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일시',
...
(
    id           BIGINT       NOT NULL AUTO_INCREMENT  COMMENT '경제 이벤트 PK',
    uid          VARCHAR(255) NOT NULL                 COMMENT 'iCal UID — 배치 upsert 기준 키',
    title        VARCHAR(255) NOT NULL                 COMMENT '이벤트 제목',
    event_date   DATE         NOT NULL                 COMMENT '이벤트 날짜 (UTC)',
    event_time   TIME         NULL                     COMMENT '이벤트 시각 (NULL = 종일 이벤트)',
    country_code VARCHAR(10)  NOT NULL                 COMMENT '국가 코드 (KR, US 등)',
    exchange     VARCHAR(20)  NULL                     COMMENT '거래소 코드 (KRX, NYSE 등) — HOLIDAY 전용',
    ticker       VARCHAR(20)  NULL                     COMMENT '종목 코드 (AAPL, 005930 등) — EARNINGS 전용',
    category     VARCHAR(20)  NOT NULL                 COMMENT '이벤트 유형 (HOLIDAY | INDICATOR | EARNINGS)',
    importance   VARCHAR(10)  NULL                     COMMENT '중요도 (HIGH | MEDIUM | LOW) — INDICATOR 전용',
